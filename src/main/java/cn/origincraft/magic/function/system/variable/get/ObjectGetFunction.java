...
public class ObjectGetFunction extends NormalFunction {
    @Override
    public FunctionResult whenFunctionCalled(SpellContext spellContext, List<FunctionResult> args) {
        if (args.isEmpty()) {
            return new ErrorResult("OBJECT_GET_FUNCTION_ARGS_ERROR", "ObjectGet don't have enough args.");
        }
        if (args.get(0) instanceof StringResult v) {
            String s=v.getString();
            if(spellContext.getContextMap().hasObject(s)){
                Object o=spellContext.getContextMap().getObject(s);
